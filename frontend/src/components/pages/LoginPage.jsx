...
function LoginPage() {
  ...
  async function handleSubmit(e) {
    ...
    console.log("Le fetch va commencer")
    ...
    fetch("http://localhost:5000/users/login/", {
      ...
      .then(res => res.json())
      .then((token) => {
        console.log("C'est quoi mon token ?", token)
        if (token.success === false) {
          toast.update(id, { render: "Your data seems wrong 🧐", type: "error", isLoading: false, hideProgressBar: false, autoClose: 5000 })
        } else {
          localStorage.setItem("token", token.token);
          setUser({ payload: token.data })
        }
      })
      .catch(err => {
        if (err) {
  ...
  function handleChange(e) {
    if (e.target.name === "remember") {
      setPostData((previousValue) => ({
        ...previousValue, [e.target.name]: !postData.remember
      }))
    } else {
      setPostData((previousValue) => ({
